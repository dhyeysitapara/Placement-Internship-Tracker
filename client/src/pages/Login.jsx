  ...
  const handleSuccess = async (response) => {
    try {
      ...
      if (err.response?.data?.details) {
        ...
        showToast('Google Sign-In was unsuccessful.', 'error');
      }
    }
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 24, padding: '8px', background: 'rgba(255,255,255,0.01)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.04)' }}>
          <GoogleLogin onSuccess={handleSuccess} onError={() => showToast('Google Sign-In failed', 'error')} theme="filled_black" shape="pill" text="continue_with" width="300" />
        </div>

        <p style={{ fontSize: 14, color: '#888', marginTop: 10 }}>
          Don't have an account? <Link to="/signup" style={{ color: BL, textDecoration: 'none', fontWeight: 600 }}>Sign up</Link>
        </p>
      </div>
    </div>
  );
